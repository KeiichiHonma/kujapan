...
<div id="initialize">
    ...
    <table class="initialize">
        <tr>
            <th class="title">{$locale.input_given_name}<br />{$locale.input_given_name_remark}</th>
            <td class="form">{if $smarty.post.given_name}{$smarty.post.given_name|escape}{/if}</td>
        </tr>
        <tr>
            <td colspan="2">
...
{*    <table class="initialize">
    ...
    <tr>
        <td align="center">
                <input type="hidden" name="given_name" value="{if $smarty.post.given_name}{$smarty.post.given_name|escape}{/if}" />
                <input type="hidden" name="mail" value="{if $smarty.post.mail}{$smarty.post.mail|escape}{/if}" />
                <input type="hidden" name="csrf_ticket" value="{$csrf_ticket}" />
                <input type="hidden" name="operation" value="" />
                <input type="image" src="/locale/{$smarty.const.LOCALE}/img/button/back_btn.jpg" value="submit" class="btn" onClick="return form_back()" />
